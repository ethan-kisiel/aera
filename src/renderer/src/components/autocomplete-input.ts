...
export class AutocompleteInput extends LitElement {
  ...
  private get _ghostText(): string {
    ...
    if (
      !suggestion ||
      !this.value ||
      !suggestion.toLowerCase().startsWith(this.value.toLowerCase())
    ) {
      return ''
    }
  ...
  private _handleArrowDown(event: KeyboardEvent): void {
    ...
    this._highlightedIndex = (this._highlightedIndex + 1) % this._matches.length
  }
  ...
  private _handleArrowUp(event: KeyboardEvent): void {
    ...
    this._highlightedIndex =
      (this._highlightedIndex - 1 + this._matches.length) % this._matches.length
  }
  ...
  private _updateMatches(): void {
    ...
    this._matches = this.items
      .filter((item) => {
         ...
         * "port" -> no match
         */
        return normalizedItem.startsWith(query) && normalizedItem !== query
      })
  ...
  private _selectFromMouse(event: MouseEvent, item: string): void {
    event.preventDefault()
