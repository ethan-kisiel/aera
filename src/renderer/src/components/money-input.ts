...
export class MoneyInput extends LitElement {
  ...
  protected override render(): TemplateResult {
    return html`
      <div class="money-input" @focusout=${this._handleFocusOut}>
  ...
  private _handleCentsInput(event: Event): void {
    ...
    const value = this._digitsOnly(input.value).slice(0, 2)
  ...
  private _tryCommit(): void {
     ...
     */
    if (this._dollars.length === 0 && this._cents.length === 0) {
      this.invalid = false
    ...
    const cents = this._cents.length > 0 ? this._cents.padEnd(2, '0') : '00'
  ...
  private _updateValidity(): void {
    ...
    this.invalid = !Number.isFinite(cents) || cents > 99
  }

  private _syncFromCommittedValue(): void {
    if (
      this.value === this._lastCommittedValue &&
      (this._dollars.length > 0 || this._cents.length > 0)
    ) {
      return
    }
     ...
     * 12.3 -> 12.299999...
     */
    const totalCents = Math.round(this.value)
  ...
  private _updateInputValues(): void {
    const dollarsInput = this.renderRoot.querySelector('.dollars') as HTMLInputElement | null

    const centsInput = this.renderRoot.querySelector('.cents') as HTMLInputElement | null
  ...
  private _focusDollars(): void {
    const input = this.renderRoot.querySelector('.dollars') as HTMLInputElement | null

    input?.focus()
  }

  private _focusCents(): void {
    const input = this.renderRoot.querySelector('.cents') as HTMLInputElement | null
  ...
  static override styles = css`
    ...
    :host([invalid]) .money-input {
      ...
      box-shadow: 0 0 0 3px color-mix(in srgb, var(--color-danger, #d92d20) 10%, transparent);
    }
