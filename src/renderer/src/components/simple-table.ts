...
export class SimpleTable<T extends object> extends LitElement {
  /**
        ...
        <table>
          ...
          <tbody>
            ${
              this.rows.length === 0
                ? this._renderEmpty()
                : this.rows.map((row) => this._renderRow(row))
            }
          </tbody>
        </table>
      </div>
  ...
  private _renderRow(row: T) {
    ...
    return html`
      <tr class=${deleted ? 'deleted' : ''}>
        ${this.columns.map((column) => this._renderCell(row, column))}
  ...
  private _focusRow(row: T): void {
    this.dispatchEvent(
      new CustomEvent<TableFocusEvent<T>>('row-focus', {
        bubbles: true,
        composed: true,
        detail: {
          row
